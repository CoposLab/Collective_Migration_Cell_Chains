...
double swell_1 = 1.04;
const double pseudopod_critL_1 = 10.0;     // critical length of pseudopod 

// CELL 2 PARAMETERS
const double k_cell_2 = 1000;
const double xi_2 = 0.05;
double swell_2 = 1.04;
const double pseudopod_critL_2 = 10.0;    // critical length of pseudopod 
...
double junction_rep = 20.0;            // R
double k_junction = 6.0;               // A
double junction_rest_L = 0.75;         // r
double junction_dist = 4.0;            // a
...
void computeAdhesionForces(int Npts, vertex Nodes[], vector2 f_adh[], double Nbonds[], double kminusTerm[], double yTerm[], double formationRate[], double adh_prop, double kminus_bond, double a_bond) {
	int k;
	double dist;
...
void computeLeaderPolymerizationForces(int Npts, vertex c_Nodes[], vector2 c_f_poly[], double c_f_correctPolymerization[], double c_Nbonds[], vector2 c_ef[], double c_prev_pfx[], double c_prev_pfy[], double setArcLength, double c_delta_tip, double c_xFront, double swell, double pseudopod_critL, double e_bond, int t, double tstep, double T_RELAX){
	...
	for(k=0; k<Npts; k++) {
		if(c_Nodes[k].def.x >= (c_xFront-c_delta_tip) && c_Nodes[k].def.y >= 0.5) {
	...
	for(k=0; k<Npts; k++) {
    if(c_Nodes[k].def.x >= (c_xFront-c_delta_tip) && c_Nodes[k].def.y >= 0.5) { 
      c_current_arcLength += c_Nodes[k].characteristicL;
      if(c_current_arcLength <= setArcLength) {
        c_Nodes[k].front = 1;
        c_count_poly++;      
        c_f_poly[k].x = swell*exp(-6.59*c_load_force)-0.1+c_load_force;
      }
    }
	...
	if(c_distFromSticky > pseudopod_critL) {
		...
		for(k=0; k<Npts; k++) {
			if(c_Nodes[k].def.x>c_frontSticky && c_Nodes[k].def.y<5.0 && c_Nodes[k].sticky == 0) { // Nodes[k].def.y<5.0 //before
				c_Nodes[k].sticky = 1;
				c_Nodes[k].bottom = 1;
				c_Nodes[k].adh_link.x = c_Nodes[k].def.x;
...
void computeTrailerPolymerizationForces(int Npts, vertex c_Nodes[], vector2 c_f_poly[], double c_f_correctPolymerization[], double c_Nbonds[], vector2 c_ef[], double c_prev_pfx[], double c_prev_pfy[], double c_prev_repx[], double c_prev_repy[], double setArcLength, double c_delta_tip, double c_xFront, double swell, double pseudopod_critL, double e_bond, double c1_xBack, double c2_junction_ymin, int t, double tstep, double T_RELAX){
	...
	if((t*tstep) > T_RELAX){
		for(k=0; k<Npts; k++) {
			if(c_Nbonds[k]==0 && c_Nodes[k].def.x<=(c_xFront-pseudopod_critL) && c_Nodes[k].def.y<0.5) {
				c_Nodes[k].sticky = 1;
				c_Nodes[k].bottom = 1;
				c_Nodes[k].adh_link.x = c_Nodes[k].def.x;
				c_Nodes[k].adh_link.y = -0.5;
...
void computeJunctionForces(int Npts, vertex c1_Nodes[], vertex c2_Nodes[], vector2 c1_f_junction[], vector2 c2_f_junction[], int c2_count_poly, double k_junction, double junction_rest_L, double junction_rep, vector2 c1_n[], vector2 c2_n[], double c2_junction_ymin, double c2_sum_fpoly, double c1_sum_prev_junction, double c2_sum_prev_junction, vector2 c1_f_repulsion[], vector2 c2_f_repulsion[], int check, vector2 c2_f_poly[]){
	...
	for(k=0; k<Npts; k++){
		...
		if(c2_Nodes[k].front == 1 && c2_Nodes[k].sticky == 0){
			dist2 = 1000.0;
			for(j=0; j<Npts; j++){
	...
	for(k=0; k<c2_count_poly; k++){
		...
		if(dist > 0.0){
			c1_f_junction[c1_idx].x += junction_rep*exp(-(dist/(junction_rest_L))) - k_junction*exp(-(dist/(junction_dist)));
			c2_f_junction[c2_idx].x += -(junction_rep*exp(-(dist/(junction_rest_L))) - k_junction*exp(-(dist/(junction_dist))));
		} else if(dist < 0.0) {
			c1_f_junction[c1_idx].x += -(junction_rep*exp(-(-dist/(junction_rest_L))) - k_junction*exp(-(-dist/(junction_dist))));
			c2_f_junction[c2_idx].x += junction_rep*exp(-(-dist/(junction_rest_L))) - k_junction*exp(-(-dist/(junction_dist)));
		}
...
void progress(int Npts, vertex c1_Nodes[], vertex c2_Nodes[]) {
  ...
  double c2_A0 = 0.;
	...
	double c1_prev_pfx[Npts], c1_prev_pfy[Npts]; 
	...
	double c2_prev_pfx[Npts], c2_prev_pfy[Npts];
    ...
    if(t==0) {
      ...
      check = 0;
		...
		// DETERMINE THE CELL-CELL CONTACT REGION POINTS
		for(k=0; k<Npts; k++){
			// CELL 1
			if(c1_Nodes[k].def.x <= c1_halfLength && c1_Nodes[k].bottom == 0){
				c1_Nodes[k].back = 1;
			}

			// CELL 2
			if(c2_Nodes[k].def.x >= (c2_xFront-c2_delta_tip) && c2_Nodes[k].sticky == 0){
		...
		if((t*tstep) > T_RELAX && (t*tstep) < T_CELL2MOVE){
			...
			computeLeaderPolymerizationForces(Npts, c1_Nodes, c1_f_poly, c1_f_correctPolymerization, c1_Nbonds, c1_ef, c1_prev_pfx, c1_prev_pfy, setArcLength_1, c1_delta_tip, c1_xFront, swell_1, pseudopod_critL_1, e_bond_1, t, tstep, T_RELAX);
			computeTrailerPolymerizationForces(Npts, c2_Nodes, c2_f_poly, c2_f_correctPolymerization, c2_Nbonds, c2_ef, c2_prev_pfx, c2_prev_pfy, c2_prev_repx, c2_prev_repy, setArcLength_2, c2_delta_tip, c2_xFront, swell_2, pseudopod_critL_2, e_bond_2, c1_xBack, c2_junction_ymin, t, tstep, T_RELAX);
		} else {
			check = 0;
		}
		...
		if((t*tstep) > T_RELAX){
			// REFORM ADHESIONS SITES 
			for(k=0; k<Npts; k++) {
				if(c1_Nbonds[k]==0 && c1_Nodes[k].def.x<=(c1_xFront-pseudopod_critL_1) && c1_Nodes[k].def.y<0.5) {
					c1_Nodes[k].sticky = 1;
					c1_Nodes[k].bottom = 1;
					c1_Nodes[k].adh_link.x = c1_Nodes[k].def.x;
					c1_Nodes[k].adh_link.y = -0.5;
					c1_Nbonds[k] = tstep*e_bond_1;
				}
				if(c2_Nbonds[k]==0 && c2_Nodes[k].def.x<=(c2_xFront-pseudopod_critL_2) && c2_Nodes[k].def.y<0.5) {
					c2_Nodes[k].sticky = 1;
					c2_Nodes[k].bottom = 1;
					c2_Nodes[k].adh_link.x = c2_Nodes[k].def.x;
					c2_Nodes[k].adh_link.y = -0.5;
					c2_Nbonds[k] = tstep*e_bond_2;
				}
			}
		}
		...
		computeAdhesionForces(Npts, c1_Nodes, c1_f_adh, c1_Nbonds, c1_kminusTerm, c1_yTerm, c1_formationRate, adh_prop_1, kminus_bond_1, a_bond_1);
		computeAdhesionForces(Npts, c2_Nodes, c2_f_adh, c2_Nbonds, c2_kminusTerm, c2_yTerm, c2_formationRate, adh_prop_2, kminus_bond_2, a_bond_2);
    ...
    computeContactForces(Npts, c2_Nodes, c2_f_contact, k_contact_2);
		...
		for(int k=0; k<Npts; k++) {
			if(c1_Nbonds[k]<=0.000001) { c1_Nbonds[k] = 0.0; c1_Nodes[k].sticky = 0; }
			if(c2_Nbonds[k]<=0.000001) { c2_Nbonds[k] = 0.0; c2_Nodes[k].sticky = 0; }
      ...
      c2_f[k].y += c2_pf[k].y*c2_Nodes[k].characteristicL;
		...
		for(int k=0; k<Npts; k++) {
   		...
   		c1_v[k].x = c1_f[k].x/(xi_1*c1_Nodes[k].characteristicL);
			...
			//printf("(%.5f,%.5f) with F (%.5f,%.5f) and velocity (%.5f,%.5f)\n",Nodes[k].def.x,Nodes[k].def.y,f[k].x,f[k].y,v[k].x,v[k].y);
